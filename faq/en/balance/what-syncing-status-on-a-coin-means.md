...
The 'syncing' status indicator on the coin shows that the tekorix app tries to synchronize itself with a given blockchain.
...
Keep in mind, tekorix app keeps the synchronization process running only when the tekorix app remains open. If the app is closed the synchronization process is paused and only resumes when the app is opened again.
