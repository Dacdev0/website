...
To delete wallets and related data from tekorix:
...
The unlink action merely deletes the private key controlling the wallet from the tekorix app. To restore deleted wallets in the future, users require a private key (mnemonic phrase) for each such wallet as well as its passphrase(if it had any). Restoring access to a deleted wallet without knowing its private key (mnemonic phrase) is impossible.
