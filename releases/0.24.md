...
Once the list above is implemented tekorix can be the only app users will ever need when it comes to NFTs.

## 2. Watch Address (Read-Only Wallets)

The tekorix app now includes the ability to track wallets/addresses on EVM blockchains like Ethereum or Binance Smart Chain. This enables someone to watch balances and monitor transaction activity for any address on any EVM blockchain, be it a user or a smart contract.

This also marks our first minor step towards read-only multi-wallets which we wanted to add for a while.

## 3.  EIP-1559

tekorix is now EIP-1559 compliant. The transaction fee recommendation service in the tekorix app now almost guarantees speedy confirmation times :)

## 4. WalletConnect v2

This version also features WalletConnect version 2.  The tekorix is one of the first wallet apps to integrate WalletConnect v2 which addresses many issues present in WalletConnect v1.
